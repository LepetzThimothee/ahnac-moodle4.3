...
/**
 * Strings for component 'simplecertificate', language 'fr', version '4.3'.
 *
...
$string['certificatetext_help'] = 'Ceci est le texte qui sera utilisé dans le certificat, certains mots spéciaux seront remplacés par des variables telles que le nom du cours, le nom de l’étudiant, la note… Ce sont : {USERNAME} -> nom d’utilisateur complet {COURSENAME} -> nom du cours complet (ou un autre nom de parcours défini) {GRADE} -> formaté année {DATE} -> date formatée  {OUTCOME} -> Résultats {HOURS} -> heures définies en cours {TEACHERS} -> Liste des enseignants
...
$string['printdate_help'] = 'C’est la date qui sera imprimée, si la date d’impression est sélectionnée. Si la date de fin de cours est sélectionnée, et que l’étudiant n’a pas terminé le cours, la date de réception sera imprimée. Vous pouvez également choisir d’imprimer la date à laquelle une activité a été notée. Si un certificat est délivré avant que l’activité ne soit terminée, la date de réception sera imprimée.';
