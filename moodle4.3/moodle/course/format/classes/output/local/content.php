...
class content implements named_templatable, renderable {
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        FROM {tag_instance} tagi
        JOIN {tag} tag
        ON tagi.tagid=tag.id
        WHERE tagi.itemtype = 'course'
        AND tagi.itemid = :courseid";
        $params = ['courseid' => $course->id];
        $tags = $DB->get_records_sql($query, $params);
        $tagsdata = [];
        foreach ($tags as $tag) {
            $tagsdata[] = ['id' => $tag->id, 'name' => $tag->name, 'tagurl' => new moodle_url("/tag/index.php?tag=$tag->name")];
        }
